...
### 한 줄 소개
비슷한 한자를 자동으로 그룹화해주는 일본어 학습 단어장

### 서비스 목적
일본어를 학습하는 사용자들이 단어를 검색하고 저장할 때, 특히 헷갈리기 쉬운 한자들을 자동으로 그룹화하여 효율적으로 학습할 수 있도록 돕는 서비스입니다. 같은 음독/훈독을 가진 한자들을 모아서 보여줌으로써, 비슷한 한자들을 한 번에 정리하고 기억할 수 있습니다.
...
### 차별점 (MVP 핵심)
1. **한자 자동 그룹화**: 단어 저장 시 비슷한 한자를 자동으로 찾아서 그룹으로 묶어줌 (다른 앱에는 없는 기능)
2. **간단한 사용법**: 검색 → 저장 → 그룹 확인 3단계로 쉽게 사용
3. **오픈소스 데이터**: 신뢰할 수 있는 JMdict, KanjiDic2 사전 데이터 활용
...
### 3.1 단어 검색 및 저장
JMdict 오픈소스 사전을 활용하여 단어를 검색하고 저장합니다.

**MVP 구현 범위**

#### 검색 기능
- 히라가나, 한자로 일본어 단어 검색
- 검색 결과: 단어, 후리가나, 뜻, 예문 1-2개
- 간단한 검색 UI (검색창 + 결과 리스트)

#### 저장 기능
- "저장" 버튼으로 단어 저장 (IndexedDB)
- 저장된 단어 목록 페이지
- 단어 삭제 기능
...
### 3.2 한자 그룹화 폴더 (핵심 차별화 기능)
비슷한 한자들을 그룹으로 묶어 체계적으로 학습할 수 있는 기능입니다. 여러 기준으로 한자를 분류하여 연관 학습 효과를 극대화합니다.

**그룹화 기준 (7가지)**

> **💡 MVP 핵심 기능**: 부수 기반(1번), 구성 요소 기반(2번) 그룹화만 우선 구현합니다. 이 두 가지가 서비스의 핵심 차별화 포인트입니다.

#### 1) 부수(部首) 기반 그룹화 ✅ MVP 핵심
...
## 6. MVP 범위 (단기 프로젝트: 1-2일)

### 핵심 기능 (필수 - 1-2일 내 완성)

#### 1. 단어 검색 및 저장 (최우선)
- ✅ **검색 기능**
  - JMdict 기반 검색 (미리 파싱된 JSON 사용)
  - 히라가나, 한자 입력 지원 (로마자는 선택)
  - 검색 결과 표시: 단어, 후리가나, 뜻, 예문 1-2개
...
#### 2. 한자 그룹화 (차별화 핵심)
- ✅ **자동 그룹 추천 (3가지 핵심 기준)**
  - 구성 요소 기반 그룹화 (1순위): 특정 한자를 포함하는 한자들 자동 그룹화
  - 부수 기반 그룹화 (2순위): 같은 부수를 가진 한자 자동 그룹화
  - 음독/훈독 동일 그룹 (3순위): 같은 음독 또는 훈독을 가진 한자 자동 그룹화
  - 단어 저장 시 자동 추천 표시
  - 사용자가 선택/거부 가능
...
**참고**: 기본 IndexedDB는 포함되므로 "저장된 단어"는 이미 로컬에서 작동함. 완전한 오프라인 지원은 추가 작업 필요.
...
**오전 (4시간): 기본 인프라**
- [ ] 프로젝트 설정 (React + Vite + TypeScript) - 30분
- [ ] JMdict 데이터 준비 (상위 1,000개 단어만) - 1시간
- [ ] KanjiDic2 데이터 준비 (상용한자 500자) - 1시간
- [ ] IndexedDB 설정 (Dexie.js) - 30분
- [ ] 기본 라우팅 및 레이아웃 - 1시간
...
- [ ] 검색 결과 표시 - 1시간
- [ ] 단어 저장 기능 - 1시간
- [ ] 단어 목록 페이지 (기본 리스트) - 1-2시간

**Day 1 완료 시점**: 검색 → 저장 → 목록 보기 작동
...
**오후 (3.5-5.5시간): 마무리**
- [ ] 암기 체크 기능 (체크박스) - 1시간
- [ ] 간단한 필터링 (전체/학습중/암기함) - 1시간
- [ ] 데이터 내보내기/가져오기 - 1시간
- [ ] UI 정리 및 스타일링 - 1시간
- [ ] 버그 수정 및 테스트 - 1-2시간

**Day 2 완료 시점**: MVP 전체 기능 작동

#### 시간 절약 팁

**데이터 준비 시간 단축**
- 전체 JMdict(20만 단어) 대신 상위 1,000~3,000개만
- 한자도 상용한자 500~1,000자로 제한
- 미리 파싱된 JSON 파일 사용 (XML 파싱 생략)
...
**총 개발 시간**: 16-20시간 (2일, 풀타임)

### MVP 성공 기준 (단기 프로젝트)

**기능 완성도 (필수)**
- ✅ 단어 검색이 작동함 (정확도 80% 이상이면 충분)
- ✅ 검색한 단어를 저장할 수 있음
- ✅ 저장된 단어 목록을 볼 수 있음
- ✅ 한자 단어 저장 시 그룹 추천이 표시됨
- ✅ 그룹별로 단어를 볼 수 있음
- ✅ 암기 체크를 할 수 있음
- ✅ 데이터를 내보내고 가져올 수 있음
...
**동작 안정성**
- ✅ 브라우저 새로고침 시 데이터 유지됨
- ✅ 치명적인 버그 없음 (앱이 깨지지 않음)
- ✅ 기본 에러 처리 (검색 실패, 저장 실패 등)

**최소 성능 (완벽하지 않아도 됨)**
- ⭕ 초기 로딩 5초 이하 (3초 이하면 더 좋음)
- ⭕ 검색 응답 500ms 이하 (100ms 이하면 더 좋음)
- ⭕ 큰 버벅임 없이 사용 가능

**사용자 경험 (기본만)**
- ⭕ 기능을 찾을 수 있음 (버튼 위치가 명확)
- ⭕ 모바일에서도 사용 가능 (반응형)
- ⭕ 에러 시 적절한 메시지 표시

**목표 (프로토타입)**
- 🎯 **데모 가능한 수준**: 다른 사람에게 보여줄 수 있음
- 🎯 **핵심 가치 증명**: 한자 그룹화가 왜 유용한지 보여줄 수 있음
- 🎯 **피드백 수집 가능**: 실제로 써보고 의견을 받을 수 있음
...
- **주요 변경사항**: 
  - MVP 범위 대폭 축소 (핵심 기능만)
  - 학습 기능, 태그, 고급 필터링 → Phase 2로 이동
  - 완전한 오프라인 지원 → 중간 우선순위로 이동
  - 개발 일정 10주 → 1-2일로 단축
